 ...
 */
export class Observer { // 它的作用是给对象的属性添加 getter 和 setter，用于依赖收集和派发更新
  value: any;
  dep: Dep;
  ...
  constructor (value: any) {
    ...
    if (Array.isArray(value)) { // 对 value 做判断，
      ...
      this.observeArray(value) // 对于数组会调用 observeArray 方法
    } else {
      this.walk(value) // 对纯对象调用 walk 方法
    }
...
export function defineReactive (
  ...
  const dep = new Dep() // 最开始初始化 Dep 对象的实例

  const property = Object.getOwnPropertyDescriptor(obj, key) // 接着拿到 obj 的属性描述符
