...
const hooks = ['create', 'activate', 'update', 'remove', 'destrioy']
...
export function createPatchFunction (backend) {
  // createPatchFunction 内部定义了一系列的辅助方法，最终返回了一个 patch 方法，
  ...
  function createElm ( // createElm 的作用是通过虚拟节点创建真实的 DOM 并插入到它的父节点中
    vnode,
    insertedVnodeQueue,
    parentElm,
    refElm,
  ...
  function createComponent (vnode, insertedVnodeQueue, parentElm, refElm) { // createComponent 方法目的是尝试创建子组件
    let i = vnode.data
    if (isDef(i)) {
  ...
  function insert (parent, elm, ref) { // 调用一些 nodeOps 把子节点插入到父节点中
    if (isDef(parent)) {
      if (isDef(ref)) {
        if (nodeOps.parentNode(ref) === parent) {
          nodeOps.insertBefore(parent, elm, ref) // 调用原生 DOM 的 API 进行 DOM 操作
        }
      } else {
  ...
  return function patch (oldVnode, vnode, hydrating, removeOnly) {
    // oldVnode 表示旧的 VNode 节点，它可以不存在或者是一个 DOM 对象
